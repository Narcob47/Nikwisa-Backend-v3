...
class StoreImageViewSet(viewsets.ModelViewSet):
    ...
    def list_by_store(self, request, store_id=None):
        """
        Fetch all images related to a specific store
        """
        try:
            store = Store.objects.get(id=store_id)  # Fetch store by store_id
            store_images = StoreImage.objects.filter(store=store)  # Filter images by store
